<title>Print Purchase Order - Supplies</title>
<?php
//convert to pdf
try {
    ob_clean();
    ...
    $mPdf->SetXY(20, $y += 5);
    $mPdf->WriteCell(170, 6, 'Purchase Order - Supplies', 0, 0, 'C');

//    $mPdf->SetXY(30,$y);
//    $mPdf->WriteCell(170,6,'','B',0,'C'); //print line
    //Merge All Supplier
    foreach ($modelDetails as $supplier) {
        $suppliers[] = $supplier->suppliers->name;
    }
    $supplierAll = implode(', ', array_unique($suppliers));
    //End of Merging All Supplier

    $y += 15;
    $mPdf->SetXY($x, $y);
    $mPdf->WriteCell(30, 6, 'Supplier:');
    $mPdf->WriteCell(120, 6, $supplierAll, 'B');
    ...
    $mPdf->SetXY($x, $y += 6);
    $mPdf->WriteCell(30, 6, 'Supplier', 'B', 0, 'C');
    $mPdf->WriteCell(30, 6, 'Supplies Code', 'B', 0, 'C');
    $mPdf->WriteCell(50, 6, 'Item Name', 'B', 0, 'C');
    ...
    foreach ($modelDetails as $detail) {
        $y += 6;
        $mPdf->SetXY($x, $y);
        $mPdf->WriteCell(30, 6, $detail->suppliers->name);
        $mPdf->WriteCell(30, 6, $detail->item_code);
        $mPdf->WriteCell(50, 6, Inventories::sql_getName_byCode($detail->item_code), '', 0, 'C');
