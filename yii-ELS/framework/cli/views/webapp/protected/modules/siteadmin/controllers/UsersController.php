...
class UsersController extends SiteAdminController {
    /**
    ...
    public function actionUpdate()
    {
        ...
        // $this->performAjaxValidation($model);
        if (isset($_POST['Users'])) {
            $modelUpdate->attributes = $_POST['Users'];
            if ($_POST['Users']['new_password'] != NULL) {
                $modelUpdate->scenario = 'updateChangePassword';
                if ($modelUpdate->validate()) {
                    $modelUpdate->updated_at = Settings::get_DateTime();
                    $modelUpdate->pword_hash = md5($_POST['Users']['new_password']);
                    $modelUpdate->role = $_POST['Users']['role'];

                    if ($_POST['Users']['role'] == Utilities::YES) {
                        $modelUpdate->account_type_id = AccountTypes::USER_ADMIN;
                    } else {
                        $modelUpdate->account_type_id = AccountTypes::USER_USER;
                    }

                    $modelUpdate->save();
                    Utilities::set_Flash(Utilities::FLASH_SUCCESS, 'Users successfully updated.');
                    $this->redirect(array('view', 'id' => $modelUpdate->id));
                } else {
                    Utilities::set_Flash(Utilities::FLASH_ERROR, Utilities::get_ModelErrors($modelUpdate->errors));
                }
            } else {
                if ($modelUpdate->validate()) {
                    $modelUpdate->updated_at = Settings::get_DateTime();
    ...
    public function actionAdmin()
    {
        ...
        $modelCreate = new Users;
        $modelCreate->scenario = 'createUser';
        Utilities::setMenuActive_Siteadmin(Settings::get_ControllerID(), Settings::get_ActionID());

        if (isset($_POST['Users'])) {

            $modelCreate->attributes = $_POST['Users'];
            $modelCreate->created_at = Settings::get_DateTime();
            $modelCreate->updated_at = Settings::get_DateTime();
            $modelCreate->is_active = Utilities::YES;
            $modelCreate->pword_hash = md5($modelCreate->confirm_password);
            $modelCreate->emp_id = $_POST['Users']['emp_id'];
            $modelCreate->role = $_POST['Users']['role'];

            if ($_POST['Users']['role'] == Utilities::YES) {
                $modelCreate->account_type_id = AccountTypes::USER_ADMIN;
            } else {
                $modelCreate->account_type_id = AccountTypes::USER_USER;
            }

            if ($modelCreate->validate()) {
                $modelCreate->save();
                Utilities::set_Flash(Utilities::FLASH_SUCCESS, 'Account successfully created.');
                $this->redirect(array('admin'));
            } else {
                Utilities::set_Flash(Utilities::FLASH_ERROR, Utilities::get_ModelErrors($modelCreate->errors));
            }
        }
