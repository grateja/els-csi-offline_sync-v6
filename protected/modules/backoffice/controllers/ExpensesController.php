<?php
    ...
    class ExpensesController extends BackOfficeController {
        /**
        ...
        public function actionSalaries() {
            ...
            if (isset($_POST['Expenses'])) {
                ...
                $model->branch_id = Settings::get_BranchID();
                $model->client_id = Settings::get_ClientID();
                $model->expenses_type_id = ExpensesTypes::EXPENSES_TYPE_SALARIES;
                $model->title = $model->expensesTypes->name;
                $model->date = date('Y-m-d', strtotime($model->date));
                $model->ref_no = Expenses::createNewRefNo();
        ...
        public function actionAdminReports() {
            ...
            $rawData = Expenses::model()->findAll(' is_deleted = :isDeleted AND branch_id = :branchID  AND date(date) between :startDate AND :endDate  order by   updated_at DESC', array(
                'endDate' => $endDate,
                'startDate' => $startDate,
