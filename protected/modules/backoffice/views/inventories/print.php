...
<?php
    ...
    $model = new CActiveDataProvider('Inventories', array(
        'criteria' => array(
            'order' => ' updated_at DESC',
            'condition' => ' is_deleted = :isDeleted AND branch_id = :branchID AND date(updated_at) between :startDate AND :endDate',
            'params' => array(
                'endDate' => $endDate,
                'startDate' => $startDate,
    ...
    try {
        ...
        $mPdf->AddPage();
        $mPdf->setTitle('Print Delivery Reports');
        ...
        $mPdf->output('Print Delivery Reports' . $details . '.pdf', "I");
        ob_end_flush();
        exit();
    } catch (Exception $ex) {
        print $ex;
