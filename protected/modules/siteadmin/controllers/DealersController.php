<?php
    ...
    class DealersController extends SiteAdminController {
        /**
        ...
        public function actionCreate() {
            ...
            if (isset($_POST['Dealers'])) {
                ...
                $fileExtension = pathinfo($_SESSION[Dealers::tbl()]['filename'], PATHINFO_EXTENSION);
                ...
                $model->file_path = 'images/dealers/';
                $model->file_pics = $newFilename;
                if ($model->validate()) {
                    $source = YiiBase::getPathOfAlias('webroot') . '/' . $_SESSION[Dealers::tbl()]['path'] . "/" . $_SESSION[Dealers::tbl()]['filename'];
                    $destination = YiiBase::getPathOfAlias('webroot') . '/' . $model->file_path . $newFilename;
//                    Utilities::debug($source, $caption);exit();
                    rename($source, $destination);
                    $model->save();
                    Utilities::set_Flash(Utilities::FLASH_SUCCESS, 'New Dealer Successfully Created');
...
//                    Utilities::debug($source, $caption);exit();
                    rename($source, $destination);
                    $model->save();
                    Utilities::set_Flash(Utilities::FLASH_SUCCESS, 'New Dealer Successfully Updated');
                    $this->redirect(array('view', 'id' => $model->id));
                } else {
                    Utilities::set_Flash(Utilities::FLASH_ERROR, Utilities::get_ModelErrors($model->errors));
                }
        ...
        public function loadModel() {
            if ($this->_model === null) {
                if (isset($_GET['id']))
                    $this->_model = Dealers::model()->findbyPk($_GET['id']);
                if ($this->_model === null)
        ...
        public function gotoCreate() {
            $this->redirect($this->createUrl('dealers/create'));
        }
        ...
        public function actionAddPhotoSubmit() {
            ...
            $path = 'images/dealers/tmp';
            $_SESSION[Dealers::tbl()]['path'] = $path;
            ...
            $_SESSION[Dealers::tbl()]['filename'] = $fileName;
